---
description: >-
  Five things people asked the openEHR Assistant to do, each from a real
  prompt, with the tool calls it made on its own: comparing and explaining
  archetypes, finding or authoring one, generating code, and writing AQL.
---
...
Cadasto keeps the first four transcripts internally; the
...
public thread, question and answer included, so it can be read at the source.

Next: [install](install.md) the server and plugin, or see the full
